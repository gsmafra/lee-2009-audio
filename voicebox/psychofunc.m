...
%      Version: $Id: psychofunc.m,v 1.2 2010/06/30 15:39:19 dmb Exp $
%
    ...
    for i=1:nmod % loop for each model type
        ...
        switch mod
            case 1
                ...
                px=repmat(alpha,nx,1)-log(px.^(-1)-1).*repmat(beta.^(-1),nx,1);
            case 2   % cumulative Gaussian function
                xtstd=norminv((qq(1,:)-qq(5,:))./pscale); % x position of target in std measure
                sig=(pscale.*normpdf(xtstd))./qq(3,:);
                px= repmat(qq(2,:)-sig.*xtstd,nx,1) + ...
                    repmat(sig,nx,1).*norminv(px);
            case 3
                                wlog=log(1-pstd);
    ...
    for i=1:nmod % loop for each model type
        ...
        switch mod
            case 1   % logistic function
                beta=sstd./(pstd.*(1-pstd));
                alpha=qq(2,:)+log((1-pstd)./pstd)./beta;
                px=(1+exp(repmat(beta.*alpha,nx,1)-x(:)*beta)).^(-1);
            case 2   % cumulative Gaussian function
